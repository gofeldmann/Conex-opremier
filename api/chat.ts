  ...
  try {
    ...
    if (!apiKey) {
      return res.status(200).json({
        reply: 'Oi, eu sou a Patrícia! A variável GEMINI_API_KEY ainda não foi encontrada no ambiente da Vercel. Por favor, adicione GEMINI_API_KEY em Settings > Environment Variables na Vercel e faça um Redeploy. 🐶🐱',
...
`;
    ...
    for (const model of modelsToTry) {
      try {
        ...
        lastError = callErr?.message || 'Falha na requisição';
      }
    }
