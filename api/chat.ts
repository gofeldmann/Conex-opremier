...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method !== 'POST') {
    return res.status(200).json({ reply: 'Método aceito somente via POST.' });
  }
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(200).json({
        reply: 'Oi, eu sou a Patrícia! A variável GEMINI_API_KEY ainda não foi encontrada no servidor da Vercel. Adicione a variável GEMINI_API_KEY no painel da Vercel (Project Settings > Environment Variables) e faça o Redeploy da aplicação. 🐶🐱',
        suggestedProducts: ['premier-nattu', 'premier-formula']
      });
    }

    const { messages, petProfile, contextProduct, customSubfamilies } = bodyData;
...
`;
    ...
    for (const modelName of models) {
      try {
        ...
        lastError = callErr?.message || 'Falha de requisição';
      }
    }
    ...
    return res.status(200).json({
      reply: 'Oi, eu sou a Patrícia! Tivemos uma oscilação momentânea: ' + (err?.message || 'Falha de execução') + '. Como posso te auxiliar com seu pet? 🐾',
      suggestedProducts: ['premier-nattu', 'premier-formula']
    });
  }
}
